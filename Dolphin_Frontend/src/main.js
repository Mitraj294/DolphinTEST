...
async function bootstrap() {
  ...
  try {
    ...
    console.debug && console.debug('Could not install extension error filter', e);
  }
...
const isGuestAccess = () => {
  ...
  const hasGuestParams =
    ...
    urlParams.has('email');
  console.debug &&
    console.debug('isGuestAccess check:', {
...
try {
  ...
  if (authToken && !isGuestAccess()) {
    try {
      ...
      try {
        ...
        console.debug && console.debug('Could not fetch subscription status on startup', err);
      }
      ...
      if (user?.role) {
        ...
        tokenMonitor.startMonitoring({
          ...
          onExpiringSoon: (seconds) => {
            console.debug &&
              console.debug(`Your session will expire in ${Math.round(seconds / 60)} minutes`);
            // You could show a toast notification here
          },
          onExpired: () => {
            console.debug && console.debug('Session expired, redirecting to login');
      ...
      console.debug && console.debug('Could not fetch current user on startup', err);
    } finally {
      app.use(router);
      app.mount('#app');
    }
  ...
  console.debug && console.debug('Failed to bootstrap app:', err);
}
