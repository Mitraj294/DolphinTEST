 ...
 */
// Legacy guest validation flow removed. Keep a small helper (no-op) used by route guards.
const tryValidateGuestForPlans = async () => {
...
router.beforeEach(async (to, from, next) => {
  ...
  if (authToken) {
    ...
    // redirect to login.
    const validated = await tryValidateGuestForPlans();
    if (validated) {
      next();
      return;
    }
