        ...
        <div class="table-card">
          ...
          <div class="table-search-header-row">
            <div class="table-search-bar-filters">
              <!-- Search Input -->
              <input class="org-search" placeholder="Search Leads ...." v-model="search" />
            ...
            >
              <img src="@/assets/images/Add.svg" alt="Add" class="leads-add-btn-icon" />
              Add New
            </button>
          </div>
          ...
          <div class="table-container">
            <div class="table-scroll">
              <table class="table">
                ...
                <!-- Table Body -->
                <tbody>
                  <tr v-for="lead in paginatedLeads" :key="lead.id || lead.email">
                    <td data-label="Contact">
                      <span class="lead-contact-link" @click="goToLeadDetail(lead)">
                        {{ lead.contact }}
                      </span>
                    </td>
                    ...
                    <td data-label="Status">
                      <span
                        class="status-badge"
                        :class="lead.status && lead.status.toLowerCase().replace(/\s+/g, '-')"
                      >
                    ...
                    <td data-label="Notes">
                      <button class="btn-view" @click="openNotesModal(lead)">
                        ...
                        <template v-else>
                          <img src="@/assets/images/AddBlack.svg" alt="Add" class="btn-view-icon" />
                          Add
                        </template>
                      </button>
                    </td>
  ...
  setup() {
    ...
    const fetchLeads = async () => {
      ...
      try {
        ...
        leads.value = response.data.map((lead) => {
          ...
          const organization =
            (lead.organization && (lead.organization.name || lead.organization)) ||
            lead.organization_name ||
            '';
          const size =
            lead.organization_size || (lead.organization && lead.organization.size) || '';
        ...
        console.debug && console.debug('Error fetching leads:', error);
        toast.add({
          severity: 'error',
          summary: 'Error',
      ...
      confirm.require({
        ...
        accept: async () => {
          try {
            ...
            console.debug && console.debug('Error deleting lead:', error);
            toast.add({
              severity: 'error',
    ...
    const updateLeadNotes = async () => {
      ...
      try {
        ...
        console.debug && console.debug('Error updating notes:', error);
        toast.add({
          severity: 'error',
          summary: 'Error',
    ...
    const filteredLeads = computed(() => {
      ...
      if (form.value.organization_size) {
        filtered = filtered.filter((lead) => lead.size === form.value.organization_size);
      }
      if (form.value.referral_source_id) {
        filtered = filtered.filter(
          (lead) => String(lead.referral_source_id) === String(form.value.referral_source_id)
        );
