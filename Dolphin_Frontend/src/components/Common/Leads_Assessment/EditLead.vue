  ...
  methods: {
    ...
    async loadLeadFromApi(leadId) {
      try {
        ...
        console.debug && console.debug('Error fetching lead data:', e);
        return false;
      }
    },
    ...
    loadLeadFromQuery() {
      ...
      if (q.lead) {
        try {
          ...
          console.debug && console.debug('Failed to parse lead query param:', e);
        }
      }
      ...
      this.form = {
        ...
        ...(!leadObj.first_name && leadObj.name
          ? {
              ...
              last_name: (leadObj.name || '').split(/\s+/).slice(1).join(' ') || '',
            }
          : {}),
        email: leadObj.email || '',
    ...
    updatePageTitle(leadObj) {
      this.$nextTick(() => {
        ...
        this.$root?.$emit('page-title-override', name ? `Edit Lead : ${name}` : 'Edit Lead');
      });
    },
    ...
    handleError(error) {
      console.debug && console.debug('Error updating lead:', error);
