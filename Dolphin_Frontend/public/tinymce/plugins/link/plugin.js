...
(function () {
  ...
  const getAnchorElement = (editor, selectedElm) => {
    selectedElm =
      selectedElm ||
      getLinksInSelection(editor.selection.getRng())[0] ||
  ...
  const isOnlyTextSelected = (editor) => {
    ...
    const isInBlockAnchor = getAnchorElement(editor).exists((anchor) =>
      anchor.hasAttribute('data-mce-block')
    );
    if (isInBlockAnchor) {
      return false;
    }
  ...
  const isImageFigure = (elm) =>
    isNonNullable(elm) && elm.nodeName === 'FIGURE' && /\bimage\b/i.test(elm.className);
  ...
  const tryProtocolTransform = (assumeExternalTargets, defaultLinkProtocol) => (data) => {
    ...
    const suggestProtocol =
      ...
      (assumeExternalTargets === 0 /* AssumeExternalTargets.OFF */ &&
        /^\s*www(\.|\d\.)/i.test(url));
  ...
  const getAnchors = (editor) => {
    ...
    return anchors.length > 0
      ? Optional.some([{ text: 'None', value: '' }].concat(anchors))
      : Optional.none();
  };
  ...
  const isSelectionOnImageWithEmbeddedLink = (editor) => {
    ...
    return (
      ...
      editor.dom.select('img', node).length === 1
    );
  };
  ...
  const getLinkInSelection = (editor) => {
    ...
    return someIf(links.length > 0, links[0]).or(
      getLinkFromElement(editor, editor.selection.getNode())
    );
  };
