...
(function () {
  ...
  const dataToHtml = (editor, dataIn) => {
    ...
    if (!data.source) {
      global$5.extend(
        data,
        htmlToData((_a = data.embed) !== null && _a !== void 0 ? _a : '', editor.schema)
      );
      if (!data.source) {
        return '';
      }
  ...
  const getValue = (data, metaData, sourceInput) => (prop) => {
    ...
    const getNonEmptyValue = (c) =>
      get$1(c, 'value').bind((v) => (v.length > 0 ? Optional.some(v) : Optional.none()));
  ...
  const unwrap = (data, sourceInput) => {
    const metaData =
      sourceInput && sourceInput !== 'dimensions' ? extractMeta(sourceInput, data).getOr({}) : {};
  ...
  const isEmbedIframe = (url, mediaDataType) =>
    isNonNullable(mediaDataType) &&
    mediaDataType === 'ephox-embed-iri' &&
    isNonNullable(matchPattern(url));
