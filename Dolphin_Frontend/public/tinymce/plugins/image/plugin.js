...
(function () {
  ...
  const register$2 = (editor) => {
    ...
    registerOption('image_list', {
      processor: (value) => {
        const valid =
          value === false || isString(value) || isArrayOf(value, isObject) || isFunction(value);
        return valid
          ? { value, valid }
          : { valid: false, message: 'Must be false, a string, an array or a function.' };
      },
      default: false,
    });
  };
  ...
  const makeItems = (info) => {
    ...
    const getDialogContainerType = (useColumns) =>
      useColumns ? { type: 'grid', columns: 2 } : { type: 'panel' };
    return flatten([
      [imageUrl],
      imageList.toArray(),
  ...
  const changeImages = (helpers, info, state, api) => {
    ...
    image.each((img) => {
      const updateAlt =
        data.alt === '' || state.prevImage.map((image) => image.text === data.alt).getOr(false);
      if (updateAlt) {
        if (img.value === '') {
      ...
      .then((results) => {
        ...
        if (results.length === 0) {
          ...
          return Promise.reject(
            (_a = results[0].error) === null || _a === void 0 ? void 0 : _a.message
          );
        } else {
          return results[0];
        }
