...
(function () {
  ...
  const fromString$1 = (hex) =>
    isHexString(hex) ? Optional.some({ value: normalizeHex(hex) }) : Optional.none();
    ...
    return (api) => {
      ...
      const init = () => {
        ...
        if (isNone) {
          ...
          boundCallback.set(
            editor.formatter.formatChanged(formatName, (match) => api.setActive(!match), true)
          );
        } else {
          api.setActive(forall(selectedCells, checkNode));
          boundCallback.set(
            editor.formatter.formatChanged(formatName, api.setActive, false, { value: formatValue })
          );
        }
      };
  ...
  const buildClassList = (classList) => {
    ...
    return Optional.some(
      buildListItems([{ text: 'Select...', value: 'mce-no-match' }, ...classList])
    );
  };
  ...
  const getSelectedCells = (table, cells) => {
    ...
    const filtered = filter$1(allCells, (cellA) =>
  ...
  const applyCellData = (editor, cells, oldData, data) => {
    ...
    if (size(modifiedData) > 0 && cells.length >= 1) {
      ...
      table(cells[0]).each((table) => {
        ...
        const styleModified =
          size(filter(modifiedData, (_value, key) => key !== 'scope' && key !== 'celltype')) > 0;
          ...
          lift2(table(startCellOrCaption), getEnd().bind(table), (startTable, endTable) => {
            if (eq(startTable, endTable)) {
              if (isCaption(startCellOrCaption)) {
                ...
                return Optional.some(
                  forMenu(getCellsFromSelection(editor), startTable, startCellOrCaption)
                );
              }
            }
            return Optional.none();
  ...
  const addButtons = (editor, selectionTargets) => {
    editor.ui.registry.addMenuButton('table', {
      ...
      fetch: (callback) =>
        callback('inserttable | cell row column | advtablesort | tableprops deletetable'),
    });
    ...
    if (editor.queryCommandSupported('mceTableApplyCellStyle')) {
      ...
      editor.ui.registry.addMenuButton('tablecellbackgroundcolor', {
        ...
        fetch: (callback) =>
          callback(buildColorMenu(editor, getTableBackgroundColorMap(editor), 'background-color')),
        onSetup: selectionTargets.onSetupCellOrRow,
      });
      editor.ui.registry.addMenuButton('tablecellbordercolor', {
        ...
        fetch: (callback) =>
          callback(buildColorMenu(editor, getTableBorderColorMap(editor), 'border-color')),
        onSetup: selectionTargets.onSetupCellOrRow,
      });
    }
  ...
  const addMenuItems = (editor, selectionTargets) => {
    ...
    if (!hasTableGrid(editor)) {
      ...
      editor.ui.registry.addNestedMenuItem('inserttable', {
        ...
        icon: 'table',
        getSubmenuItems: () => [
          { type: 'fancymenuitem', fancytype: 'inserttable', onAction: insertTableAction },
        ],
        onSetup: onSetupEditable(editor),
      });
    }
    ...
    if (editor.queryCommandSupported('mceTableApplyCellStyle')) {
      ...
      editor.ui.registry.addNestedMenuItem('tablecellbackgroundcolor', {
        ...
        getSubmenuItems: () =>
          buildColorMenu(editor, getTableBackgroundColorMap(editor), 'background-color'),
        onSetup: selectionTargets.onSetupCellOrRow,
      });
      editor.ui.registry.addNestedMenuItem('tablecellbordercolor', {
        ...
        text: 'Border color',
        getSubmenuItems: () =>
          buildColorMenu(editor, getTableBorderColorMap(editor), 'border-color'),
        onSetup: selectionTargets.onSetupCellOrRow,
      });
    }
