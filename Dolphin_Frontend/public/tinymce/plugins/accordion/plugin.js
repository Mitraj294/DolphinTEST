...
(function () {
  ...
  const fireToggleAccordionEvent = (editor, element, state) =>
  ...
  const isSummary = (node) =>
    (node === null || node === void 0 ? void 0 : node.nodeName) === 'SUMMARY';
  const isDetails = (node) =>
    (node === null || node === void 0 ? void 0 : node.nodeName) === 'DETAILS';
  ...
  const isInsertAllowed = (editor) =>
    !isInSummary(editor) &&
    editor.dom.isEditable(editor.selection.getNode()) &&
    !editor.mode.isReadOnly();
  const getSelectedDetails = (editor) =>
  ...
  const normalizeDetails = (editor) => {
    global$3.each(
      global$3.grep(editor.dom.select('details', editor.getBody())),
  ...
  const toggleAllAccordions = (editor, state) => {
    ...
    each$1(accordions, (accordion) =>
      toggleDetailsElement(
        accordion,
        state !== null && state !== void 0 ? state : !isOpen(accordion)
      )
    );
  ...
  const getClassList = (node) => {
    var _a, _b;
    return (_b = (_a = node.attr('class')) === null || _a === void 0 ? void 0 : _a.split(' ')) !==
      null && _b !== void 0
      ? _b
      : [];
  };
  ...
  const isAccordionBodyWrapperNode = (node) =>
    ...
    contains(getClassList(node), accordionBodyWrapperClass);
      ...
      parser.addNodeFilter(accordionTag, (nodes) => {
        ...
        for (let i = 0; i < nodes.length; i++) {
          ...
          if (isAccordionDetailsNode(node)) {
            ...
            const newWrapperNode = hasWrapperNode
              ? wrapperNode
              : new global$2(accordionBodyWrapperTag, 1);
  ...
  const setupEnterKeyInSummary = (editor) => {
    editor.on('keydown', (event) => {
      if (
        (!event.shiftKey && event.keyCode === global$1.ENTER && isInSummary(editor)) ||
        isAtDetailsStart(editor)
      ) {
