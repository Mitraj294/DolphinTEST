...
(function () {
  ...
  const collectTextToBoundary = (dom, section, node, rootNode, forwards) => {
    ...
    const rootBlock =
      (_a = dom.getParent(rootNode, dom.isBlock)) !== null && _a !== void 0 ? _a : dom.getRoot();
      ...
      endNode,
      {
        ...
        cef: (node) => {
          ...
          return sort(sections, (a, b) =>
            documentPositionPreceding(a.elements[0].dom, b.elements[0].dom) ? 1 : -1
          );
        },
      },
      false
  ...
  const findAndMarkInSelection = (dom, pattern, selection, replacementNode) => {
    ...
    const textSections =
      nodes.length > 0 ? fromNodes(dom, nodes) : fromRng(dom, selection.getRng());
  ...
  const open = (editor, currentSearchState) => {
    ...
    dialogApi.set(
      editor.windowManager.open(getDialogSpec(false, initialData), { inline: 'toolbar' })
    );
  };
