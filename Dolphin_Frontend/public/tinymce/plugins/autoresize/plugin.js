...
(function () {
  ...
  const parseCssValueToInt = (dom, elm, name, computed) => {
    ...
    const value = parseInt(
      (_a = dom.getStyle(elm, name, computed)) !== null && _a !== void 0 ? _a : '',
      10
    );
  ...
  const shouldScrollIntoView = (trigger) => {
    ...
    if (
      (trigger === null || trigger === void 0 ? void 0 : trigger.type.toLowerCase()) ===
      'setcontent'
    ) {
  ...
  const resize = (editor, oldSize, trigger, getExtraMarginBottom) => {
    ...
    const resizeBottomMargin = getExtraMarginBottom
      ? getExtraMarginBottom()
      : getAutoResizeOverflowPadding(editor);
    const minHeight =
      (_a = getMinHeight(editor)) !== null && _a !== void 0 ? _a : editor.getElement().offsetHeight;
    ...
    if (
      resizeHeight !== old.totalHeight &&
      (contentHeight - resizeBottomMargin !== old.contentHeight || !old.set)
    ) {
