...
(function () {
  ...
  const hasProto = (v, constructor, predicate) => {
    ...
    if (predicate(v, constructor.prototype)) {
      ...
      return (
        ((_a = v.constructor) === null || _a === void 0 ? void 0 : _a.name) === constructor.name
      );
    }
  };
  const typeOf = (x) => {
  ...
  class Optional {
    ...
    getOrDie(message) {
      if (!this.tag) {
        throw new Error(
          message !== null && message !== void 0 ? message : 'Called getOrDie on None'
        );
      } else {
        return this.value;
      }
  ...
  const detectBrowser$1 = (browsers, userAgentData) => {
    return findMap(userAgentData.brands, (uaBrand) => {
      ...
      return find$1(browsers, (browser) => {
        var _a;
        return (
          lcBrand === ((_a = browser.brand) === null || _a === void 0 ? void 0 : _a.toLowerCase())
        );
      }).map((info) => ({
        current: info.name,
  ...
  const ancestor$1 = (scope, selector, isRoot) =>
    ancestor$2(scope, (e) => is$1(e, selector), isRoot);
  ...
  const isTextNodeWithCursorPosition = (el) =>
    getOption(el)
      ...
      .isSome();
  const isContentEditableFalse = (elem) =>
    isHTMLElement(elem) && get$b(elem, 'contenteditable') === 'false';
  ...
  const exactFromRange = (simRange) =>
    adt$4.exact(simRange.start, simRange.soffset, simRange.finish, simRange.foffset);
  ...
  const caretRangeFromPoint = (doc, x, y) => {
    var _a;
    return Optional.from(
      (_a = doc.caretRangeFromPoint) === null || _a === void 0 ? void 0 : _a.call(doc, x, y)
    );
  };
  ...
  const doGetExact = (selection) => {
    if (selection.anchorNode === null || selection.focusNode === null) {
      ...
      return after$5(anchor, selection.anchorOffset, focus, selection.focusOffset)
        ? Optional.some(
            SimRange.create(anchor, selection.anchorOffset, focus, selection.focusOffset)
          )
        : readRange(selection);
    }
  };
  ...
  const get$3 = (win) =>
    getExact(win).map((range) =>
      SimSelection.exact(range.start, range.soffset, range.finish, range.foffset)
    );
  ...
  const justCols = (warehouse) =>
    map$1(Warehouse.justColumns(warehouse), (column) => Optional.from(column.element));
        ...
        (column) => {
          if (isValidColumn(column)) {
            ...
            return getDimension(
              ...
              backups,
              colFilter,
              (cell) => fallback(Optional.some(Math.round(get$7(cell)))),
              fallback
            );
          }
        },
  ...
  const getLockedColumnsWithinBounds = (startAddress, rows, lockedColumns) =>
    filter$2(
      lockedColumns,
      (colNum) =>
        colNum >= startAddress.column && colNum <= cellLength(rows[0]) + startAddress.column
    );
  ...
  const elementFromGrid = (grid, row, column) => {
    ...
    return (
      Optional.from(
        (_b = (_a = rows[row]) === null || _a === void 0 ? void 0 : _a.cells[column]) === null ||
          _b === void 0
          ? void 0
          : _b.element
      )
  ...
  const makeRowsSection =
    (section, applyScope) => (initialGrid, details, comparator, genWrappers, tableSection) => {
      const rows = uniqueRows(details);
  ...
  const lastColumnIsLocked = (warehouse, details) =>
    exists(
      details,
      (detail) => detail.column + detail.colspan >= warehouse.grid.columns && detail.isLocked
    );
  ...
  const BarManager = (wire) => {
    ...
    const isResizer = (target) =>
      has(target, 'ephox-snooker-resizer-bar') || has(target, 'ephox-dragster-blocker');
  ...
  const getPixelForcedWidth = (editor) => {
    ...
    const parentBlock =
      (_a = dom.getParent(editor.selection.getStart(), dom.isBlock)) !== null && _a !== void 0
        ? _a
        : editor.getBody();
  ...
  const register = (editor) => {
    ...
    registerOption('table_column_resizing', {
      processor: (value) => {
        ...
        return valid
          ? { value, valid }
          : { valid: false, message: 'Must be preservetable, or resizetable.' };
      },
  ...
  const mergable = (table, selectedCells, ephemera) => {
    if (selectedCells.length <= 1) {
      ...
      return retrieveBox(table, ephemera.firstSelectedSelector, ephemera.lastSelectedSelector).map(
        (bounds) => ({ bounds, cells: selectedCells })
      );
    }
  };
  ...
  const Selections = (lazyRoot, getStart, selectedSelector) => {
    const get = () =>
      retrieve(lazyRoot(), selectedSelector).fold(() => getStart().fold(none, single), multiple);
    return {
      get,
    };
  };
    ...
    if (!eq$1(start, finish)) {
      return identify(start, finish, isRoot).bind((cellSel) => {
        ...
        if (boxes.length > 1) {
          ...
          return Optional.some(
            Response.create(Optional.some(makeSitus(start, 0, start, getEnd(start))), true)
          );
        } else {
          return Optional.none();
        }
  ...
  const adjustDown = (bridge, element, guessBox, original, caret) => {
    ...
    if (Math.abs(guessBox.bottom - original.bottom) < 1) {
      ...
      return inOutsideBlock(bridge, element, caret)
        ? adt.retry(translate(lowerCaret, JUMP_SIZE, 0))
        : adt.none();
    }
  };
  const adjustUp = (bridge, element, guessBox, original, caret) => {
    ...
    if (Math.abs(guessBox.top - original.top) < 1) {
      ...
      return inOutsideBlock(bridge, element, caret)
        ? adt.retry(translate(higherCaret, JUMP_SIZE, 0))
        : adt.none();
    }
  };
  const upMovement = {
  ...
  const handle = (bridge, isRoot, direction) => {
    return findSpot(bridge, isRoot, direction).bind((spot) => {
      ...
      return scan(bridge, isRoot, spot.element, spot.offset, direction, MAX_RETRIES).map(
        bridge.fromSitus
      );
    });
  };
    ...
    return precheck(initial, isRoot).orThunk(() => {
      return simulate(bridge, isRoot, direction, initial, anchor).map((info) => {
        ...
        return Response.create(
          Optional.some(makeSitus(range.start, range.soffset, range.finish, range.foffset)),
          true
        );
      });
    });
  ...
  const getSelectionFromSelector = (selector) => (initCell, isRoot) => {
    ...
    const cell =
      cellName === 'col' || cellName === 'colgroup' ? getSelectionCellFallback(initCell) : initCell;
    ...
    editor.on('BeforeGetContent', (e) => {
      const multiCellContext = (cells) => {
        ...
        extractSelected(cells).each((replicaElements) => {
          const content =
            e.format === 'text'
              ? getTextContent(editor, replicaElements)
              : serializeElements(editor, replicaElements);
          e.content = content;
        });
      };
      if (e.selection === true) {
      ...
      if (e.selection === true && e.paste === true) {
        ...
        head(selectedCells).each((cell) => {
          table(cell).each((table) => {
            ...
            if (
              shouldMergeContentOnPaste(editor) &&
              elements.length === 1 &&
              isTable(elements[0])
            ) {
              e.preventDefault();
  ...
  const getSelectionStartCellOrCaption = (editor) =>
    getSelectionCellOrCaption(getSelectionStart(editor), getIsRoot(editor)).filter(
      isInEditableContext$1
    );
  ...
  const registerCommands = (editor, actions) => {
    ...
    each$1(
      {
        ...
        mceTableColType: actOnType((type) =>
          type === 'th' ? actions.makeColumnsHeader : actions.unmakeColumnsHeader
        ),
    ...
    editor.addCommand('mceTableApplyCellStyle', (_ui, args) => {
      ...
      const validArgs = filter$1(
        args,
        (value, style) => editor.formatter.has(getFormatName(style)) && isString(value)
      );
  ...
  const TableResizeHandler = (editor) => {
    ...
    const afterCornerResize = (table, origin, width, height) => {
      ...
      if (width !== startW && startRawW !== '') {
        ...
        const col =
          isPreserveTableColumnResizing(editor) || isRightEdgeResize ? getNumColumns(table) - 1 : 0;
